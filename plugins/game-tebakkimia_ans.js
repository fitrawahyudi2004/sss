...
export async function before(m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !m.text || !/Ketik.*tekim/i.test(m.quoted.text) || /.*hsus/i.test(m.text))
        return !0
    ...
    if (m.quoted.id == this.tebakkimia[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.result.lambang.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += this.tebakkimia[id][2]
            conn.reply(m.chat, `*Benar!*\n+${this.tebakkimia[id][2]} XP`, m)
            clearTimeout(this.tebakkimia[id][3])
            delete this.tebakkimia[id]
        } else if (similarity(m.text.toLowerCase(), json.result.lambang.toLowerCase().trim()) >= threshold)
            m.reply(`*Dikit Lagi!*`)
        else
